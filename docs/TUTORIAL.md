...
| 用户名社交平台扫描 | **3164 个**平台 HTTP 探测（Maigret + Sherlock + WhatsMyName），**100 线程并发 + 内容关键词** | 是 |
...
- 默认 **100 线程并发**（v1.2 起 Sherlock-inspired 提速），~30 秒完成 3164 平台全扫描；可用 `--workers N` 调节（1-200）
...
# → johndoe / doe.john / j.doe / jdoe / jd / ... 共 22 个
...
**变形规则**：
- 多片段（"first last"）→ 全拼 / 点分 / 下划线 / 连字符 / 首字母 + 全名 等组合
...
```bash
pip install "spyeyes[pdf]"
```

然后给 `--save` 一个 `.pdf` 后缀的文件名即可：

```bash
python3 -m spyeyes user torvalds --save investigation.pdf
python3 -m spyeyes ip 8.8.8.8 --save ip_report.pdf --lang en
python3 -m spyeyes whois example.com --save whois.pdf
```

PDF 自动按分类分小节（用户名扫描）或字段表格（IP/WHOIS/MX/Phone/Email），用户输入字段全部 escape 防止注入。
