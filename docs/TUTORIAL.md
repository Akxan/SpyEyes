...
5. [功能详解](#功能详解)
   ...
   - [⑥ 域名 MX 记录](#-域名-mx-记录)
   - [⑦ 邮箱有效性检查](#-邮箱有效性检查)
   - [⑧ 子域名枚举（v1.3.0）](#-子域名枚举v130)
   - [⑨ 域名邮箱枚举（v1.4.0）](#-域名邮箱枚举v140)
...
| 子域名枚举（v1.3.0） | crt.sh + CertSpotter + HackerTarget + AlienVault OTX（被动多源）+ DNS A/AAAA/CNAME 验证 + HTTP probe | 是 |
| 域名邮箱枚举（v1.4.0） | crt.sh CT 日志 + WHOIS 联系人 + 深度爬虫(robots.txt + sitemap.xml + BFS 内部链接)+ 模式生成 + 可选 SMTP 验证 | 是 |
...
1. **被动汇总(并发 4 个公开数据源)**:
   ...
   - [AlienVault OTX](https://otx.alienvault.com) —— passive_dns 端点
   - [ThreatCrowd](https://www.threatcrowd.org) —— domain report
