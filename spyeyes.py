...
# 注：之前有 _REDOS_RE 启发式检测嵌套量词以防 catastrophic backtracking，
# 但在合法 regex（如 [a-z]+(-[a-z]+)*）上误报严重。已删除：
#   - data/platforms.json 是 contributor-controlled，恶意 regex 应在 PR review 拦截
#   - Maigret + Sherlock 上游也无 ReDoS 防护
#   - 真正稳健的方案需要 process-level timeout，引入复杂度过大
